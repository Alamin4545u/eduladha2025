...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const SUPABASE_URL = 'https://fwikhcittcaadwziitdv.supabase.co';
    const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ3aWtoY2l0dGNhYWR3emlpdGR2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMxMzY0MjIsImV4cCI6MjA3ODcxMjQyMn0.wLBcy29HNFPZ5dRzJ11CaIjD_zQvBi3vNLE66HlI2es';
    ...
    async function fetchUserData() {
        ...
        if (data) {
            userData = data;
        } else if (error && error.code === 'PGRST116') { // User does not exist, create a new one
            ...
            return handleError("ব্যবহারকারীর ডেটা লোড করতে ব্যর্থ হয়েছে।", error);
        }
        updateUI();
    }
    ...
    function updateUI() {
        ...
        const spinsTodayCount = userData.spins_today?.date === today ? userData.spins_today.count : 0;
    ...
    // --- Event Listeners and UI functions (Most of these remain the same) ---
    // updateWalletUI, setupEventListeners, showScreen, getInitials, createSvgWheel, generateUniqueAffiliateCode are unchanged.
    ...
    async function startQuiz() {
        const today = new Date().toISOString().slice(0, 10);
        let currentQuizProgress = userData.quiz_progress || {};
        ...
        if (currentQuizProgress.completedToday >= quizConfig.dailyLimit) {
            return tg.showAlert(`আপনি আজকের জন্য আপনার সব কুইজ সম্পন্ন করেছেন।`);
        }
    ...
    // displayCurrentQuiz and handleOptionSelect are unchanged
    function displayCurrentQuiz() {
        const today = new Date().toISOString().slice(0, 10);
        let completedToday = 0;
        let currentStep = 0;
    
        if(userData.quiz_progress?.date === today) {
            completedToday = userData.quiz_progress.completedToday || 0;
            currentStep = userData.quiz_progress.currentStep || 0;
        }
        ...
        if (currentStep >= clickTarget) return showScreen('home-screen');
    ...
    async function handleNextQuiz() {
        if (!selectedQuizOption || selectedQuizOption.textContent !== quizQuestions[currentQuizIndex].correct_answer) {
            return tg.showAlert('ভুল উত্তর! অনুগ্রহ করে সঠিক উত্তরটি নির্বাচন করুন।');
        }
        
        quizScreenElements.nextBtn.disabled = true;
        const today = new Date().toISOString().slice(0, 10);
        const currentProgress = userData.quiz_progress?.date === today ? userData.quiz_progress : { date: today, completedToday: 0, currentStep: 0 };
        const isClickTask = currentProgress.currentStep === quizConfig.clickTarget - 1;
        tg.HapticFeedback.impactOccurred('light');
        
        try {
            await window.showGiga();
            if (isClickTask) {
                // ... (Ad click handling logic remains the same)
                 setTimeout(async () => {
                    // Assuming ad was clicked for simplicity in this refactor
                    tg.HapticFeedback.notificationOccurred('success');
                    const updates = {
                        quizProgress: { date: today, completedToday: currentProgress.completedToday + 1, currentStep: 0 }
                    };
                    await awardEarnings(quizConfig.reward, updates);
    ...
    async function spinFinished(targetSegment) {
        ...
        try {
            ...
            const today = new Date().toISOString().slice(0, 10);
            const currentCount = userData.spins_today?.date === today ? userData.spins_today.count : 0;
            
            await awardEarnings(reward, { spinsToday: { date: today, count: currentCount + 1 } });
    ...
    async function handleSubmitWithdraw() {
        ...
        if (!methodSelect || !accountNumberInput) return handleError("পেমেন্ট ফর্ম সঠিকভাবে লোড হয়নি।");
    ...
    async function handleAffiliateSignup(referrerId, newUserId) {
        ...
        await supabase.from('users').update({
            balance: userData.balance + bonus, // This is a potential race condition. Better to use RPC.
            referral_earnings: userData.referral_earnings + bonus,
            referrals: [...(userData.referrals || []), newUserId]
        }).eq('telegram_id', referrerId);
    }

    // New: Award earnings function using Supabase RPC
    async function awardEarnings(amount, additionalUpdates = {}) {
        ...
        if (error) {
            handleError("আর্নিং যোগ করতে সমস্যা হয়েছে।", error);
        }
    }
});
