...
const walletElements = { balance: document.getElementById('withdrawBalance'), withdrawMethod: document.getElementById('withdrawMethod'), accountNumber: document.getElementById('accountNumber'), submitBtn: document.getElementById('submitWithdrawBtn') };
...
const withdrawConfigDoc = await db.collection('settings').doc('withdrawConfig').get();
if (withdrawConfigDoc.exists) withdrawConfig = withdrawConfigDoc.data().methods || [];
if (withdrawConfig.length === 0) {
withdrawConfig = [{name: 'bKash', placeholder: 'e.g., 01700000000'}];
}
} catch (error) {
...
walletElements.withdrawMethod.addEventListener('change', updatePlaceholder);
}
...
const select = walletElements.withdrawMethod;
select.innerHTML = '';
withdrawConfig.forEach(method => {
...
const selected = walletElements.withdrawMethod.value;
const method = withdrawConfig.find(m => m.name === selected);
if (method) {
walletElements.accountNumber.placeholder = method.placeholder || '';
document.querySelector('label[for="accountNumber"]').textContent = `${method.name} নম্বর:`;
}
}
...
const method = walletElements.withdrawMethod.value;
const account = walletElements.accountNumber.value.trim();
if (account.length < 11 || !/^\d+$/.test(account)) {
tg.showAlert('অনুগ্রহ করে সঠিক অ্যাকাউন্ট নম্বর দিন (কমপক্ষে ১১ সংখ্যা)।');
return;
}
if (userData.balance < MINIMUM_WITHDRAW_AMOUNT) {
tg.showAlert(`উইথড্র করতে ন্যূনতম ৳${MINIMUM_WITHDRAW_AMOUNT} প্রয়োজন।`);
return;
}
this.disabled = true;
...
this.disabled = false;
walletElements.accountNumber.value = '';
});
}
